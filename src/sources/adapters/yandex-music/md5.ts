/** Минимальный MD5 для подписи download URL Яндекс Музыки. */
export function md5Hex(input: string): string {
  ...
  const addUnsigned = (a: number, b: number) => {
    ...
    const bHi = b >>> 16
    const sumLo = aLo + bLo
    const sumHi = aHi + bHi + (sumLo >>> 16)
    return ((sumHi & 0xffff) << 16) | (sumLo & 0xffff)
  }
